...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState: pacman.GameState):
        ...
        self.start_state = startingGameState
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if newPos in self.corners:
                    corner_data = list(corner_data)
                    corner_data[self.corners.index(newPos)] = True
                    corner_data = tuple(corner_data)
                
                nextState = (newPos, corner_data)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if CHOSEN == 0: # 349????
        ...
        # print("State: ", state)
        # corners = [print(corner) for corner in corners]
        # print(state[1])

        corners = [corners[i] for i in range(len(corners)) if not state[1][i]]
        # print("New Corners: ", corners)
        distances = []
        for corner in corners:
            dist = mazeDistance(state[0], corner, problem.start_state)
        ...
        if len(distances) == 1:
            return distances[0]
        if len(distances) == 2:
            return min(distances[0], distances[1]) + mazeDistance(corners[0], corners[1], problem.start_state)
        else:
            return min(distances)*(len(corners))
            
    
    elif CHOSEN == 1: # 1508
        corners = problem.corners_visited
        coords, _ = state
        if len(corners) == 0:
            return 0
        else:
            ...
            for corner in corners.keys():
                h.append(mazeDistance(coords, corner, problem.start_state))
            h = min(h)
            #print("Heuristic: ", h)
            return h
