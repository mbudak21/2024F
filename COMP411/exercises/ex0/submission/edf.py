...
class BCELoss(CompNode):
    def __init__(self,p,y):
        if not isinstance(p,CompNode):
            print('BCELoss probability vector is not a CompNode')
            raise ValueError
        CompNodes.append(self)
        self.p = p
        self.y = y
    ...
    def forward(self):
        ...
        p = self.p.value
        y = self.y.value
        # Epsilon val to prevent log(0)
        epsilon = 1e-10
        # BCE Loss Formula
        self.value = - (y * np.log(p + epsilon) + (1 - y) * np.log(1 - p - epsilon))
    ...
    def backward(self):
        ...
        # Epsilon val to prevent log(0)
        epsilon = 1e-10
        # Derivative of BCE with respect to p
        grad = (p - y) / ((p * (1 - p)) + epsilon)
        self.p.addgrad(grad/len(y))